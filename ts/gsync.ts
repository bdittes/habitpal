...
let gisInited = false;
let driveFileId: string | null = null; // To store the ID of our data file
...
async function tokenResponseCallback(tokenResponse: google.accounts.oauth2.TokenResponse) {
...
async function findOrCreateFileId() {
  if (driveFileId) return driveFileId; // Return cached ID if we have it
...
async function saveFileContent(fileId: string | null, jsonData: string) {
